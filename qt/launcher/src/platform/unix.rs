...
pub fn ensure_glibc_supported() -> Result<()> {
    ...
    let get_glibc_version = || -> Option<(u32, u32)> {
        // On musl builds, gnu_get_libc_version() is not available
        // We check if it's available at compile time
        #[cfg(target_env = "gnu")]
        {
        ...
        {
            // For musl or other libc implementations, assume compatibility
            // or return a default version that passes the check
            Some((2, 17)) // Return a safe default version
        }
    };
